...
In this monorepo, every Modal `deploy.py` pins **`tongflow==0.1.0`** (PyPI) in its `pip_install`, matching the version in [`pyproject.toml`](pyproject.toml). Bump that pin in every plugin when you publish a new release.
