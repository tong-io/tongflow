...
/**
 * Plugins registry generated by `pnpm plugins:sync` (Python scan of `plugins/*`).
 *
...
export const ModalPluginConfigSchema = z.object({
    ...
    /** Relative to repo root, e.g. `plugins/tongflow-modal-Qwen3-ASR` */
    ...
    methodsByNodeSlot: z.record(
        z.string().min(1),
        ModalMethodSchema,
    ),
});

export const LlmEngineSchema = z.enum(["openrouter", "openai", "gemini"]);

export const LlmMethodSchema = z.object({
    engine: LlmEngineSchema,
    /** Reserved for future; kept for forward-compat. */
    mode: z.string().min(1).optional(),
});

export const LlmPluginConfigSchema = z.object({
    /** nodeSlot -> which LLM engine/provider to call */
